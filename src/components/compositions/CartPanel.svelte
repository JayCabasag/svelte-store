<script>
	...
	export let openCartPanel = false;

	$: overallAmount = $cartStore.reduce((accumulator, currentValue) => {
...
{#if openCartPanel}
	<div
		transition:slide={{ delay: 150, duration: 250, easing: quintOut, axis: 'x' }}
		class="z-10 absolute top-0 bg-white right-0 w-screen md:w-[450px] h-screen transition-all"
	>
		<div class="flex w-full h-[60px] p-4 bg-white border-l-2 shadow-md">
			<button on:click={handleOnCloseModal}>
				<svg
					fill="#212121"
					height="26px"
					width="26px"
					...
					xml:space="preserve"
					stroke="#000000"
					stroke-width="0.004914150000000001"
			...
			<div class="flex-1 flex items-center justify-center">
				<h1 class="text-2xl text-gray-800 font-bold">Cart</h1>
			</div>
		</div>

		<div
			class="overflow-auto h-[calc(100vh-60px-60px)] border-t border-t-green-500 p-[15px] flex flex-col gap-[15px]"
		>
		...
		<div
			class="flex items-center justify-between px-[15px] h-[60px] bg-white border-t border-t-green-500"
		>
			<p class="font-bold">Total Amount : $ {overallAmount.toFixed(2)}</p>
			<button class="bg-green-500 px-3 py-1 rounded-md font-bold text-white shadow-md"
				>Checkout</button
			>
		</div>
	</div>
